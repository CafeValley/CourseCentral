...
maincheck("ReportForAllGroupData");
function teacherdatahander($teacher_Id)
{
    global $link;

    // If the value is not numeric, it's likely already a teacher name; return as-is
    if (!is_numeric($teacher_Id)) {
        return $teacher_Id;
    }

    $stmt = $link->prepare("SELECT `id`, `name`, `name2`, `name3` FROM `teachers` WHERE `id` = ?");
    if ($stmt) {
        $id = (int)$teacher_Id;
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $resultSNC = $stmt->get_result();
        if ($resultSNC && $resultSNC->num_rows > 0) {
            $rowSNC = $resultSNC->fetch_assoc();
            $Fname = $rowSNC['name'] . " " . $rowSNC['name2'];
            $Sirname = " " . $rowSNC['name3'];
            $FullName = trim($Fname . " " . $Sirname);
            $stmt->close();
            return ($FullName);
        }
        $stmt->close();
                                                                                ...
                                                                                <form action="markentryform.php" method="POST">
                                                                                    ...
                                                                                    */
                                                                                    ...
                                                                                    <table class="table table-striped table-bordered">
                                                                                        ...
                                                                                        $resultGRR = mysqli_query($link, "SELECT `group_id`, `level_id`, `group_time`, `group_teacher`, `group_day`, `group_startday`, `group_C_date` FROM `group` WHERE 1");
                                                                                        $x = 0;
                                                                                        while($rowGRR = mysqli_fetch_array($resultGRR))
                                                                                        {
                                                                                            ...
                                                                                            <tr>
                                                                                                ...
                                                                                                <th><center><?php echo teacherdatahander($GroupTeNa);?></center></th>
