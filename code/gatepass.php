...
$regissql = mysqli_query($link,"SELECT  `level_id`, `group_id`, `paid_fees`, `discount`, `status`, `st_id`, `regis_date` 
FROM `registration` WHERE `regis_id` = $Rgisid");
$regisreturn = mysqli_fetch_array($regissql);

$levelid = $regisreturn['level_id'];
$groupid = $regisreturn['group_id'];
$studentid = $regisreturn['st_id'];
...
<table width='50%' border = '1'>
    ...
    <tr><td>Registrations Fees: <?php echo $Rfeed;?></td></tr>
    <tr><td>Tuition Fees: <?php echo $totalWanted;?></td></tr>
    <tr><td>Remainder: <?php echo $AllowToGetMark[1];?></td></tr>
    ...
    <p>
        هذا الكرت يسمح لك بالدراسة في الفترة المذكورة اعلاه. لا يوجد استرداد للأموال .

        لحفظ اي اموال الرجاء استخراج ايصال بالتاجيل. يسري من تاريخ استخراج ايصال.
    </p>


    <p>
        This receipt is valid only for the period indicated above . No refund is possible .

        To save any amounts - please issue a freeze. 
    <p>
        <strong>Facebook</strong>: short course at comboni 
         <?php
         for ($i = 0 ; $i<49 ; $i++) 
            echo "&nbsp;"; 
         ?> 
         <strong>Website</strong>: www.courses-ccst.com
        </p>
    </p>
